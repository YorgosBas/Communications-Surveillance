...
public class Registry {
    HashMap<String, Suspect> cpSuspectList = new HashMap<>();
    ...
    ArrayList<String> suspectNameList = new ArrayList<>();
    ArrayList<String> suspectCodeNameList = new ArrayList<>();
    ...
    public void addSuspect(Suspect aSuspect){
        ...
        cpSuspectList.put(aSuspect.name,aSuspect);
        addNameOfSuspects(aSuspect);
        addCodeNameOfSuspects(aSuspect);
    }

    public void addNameOfSuspects(Suspect aSuspect){
        suspectNameList.add(aSuspect.getName());
    }

    public void addCodeNameOfSuspects(Suspect aSuspect){
        suspectCodeNameList.add(aSuspect.getCodeName());
    }
    ...
    public ArrayList<String> getMessagesBetween(String number1, String number2){
        ...
        for(Communication comm : commList){ //iterate communications
            if(comm instanceof SMS && ((comm.number1.equals(number1) && comm.number2.equals(number2)) || (comm.number1.equals(number2) && comm.number2.equals(number1)))){
                sms.add(((SMS) comm).message);
            }
        }
        //remove sms not containing the given keywords
        sms.removeIf(aSms -> !(aSms.contains("Bomb") | aSms.contains("Attack") | aSms.contains("Explosives") | aSms.contains("Gun")));
    ...
    public ArrayList getSuspectList(){
        return suspectList;
    }

    public List<String> getAllNumbersOfSpecificSuspect(int index) {
        return suspectNumbers.get(index).subList(0, (suspectNumbers.get(index).size()));
    }
}
